...
└── src/                    # 源码核心工作目录
    ...
    ├── assets/             # 静态资源目录（包含精美场景壁纸、应用图标等，构建时由 Vite 统一打包与 Hash 校验存储）
...
## 🖼️ 如何添加自定义预设壁纸 (Add Custom Preset Wallpapers)

如果你想在主页的**控制中心 (Control Center)** 中添加自己的壁纸选项，请按照以下步骤操作：

### 1. 准备并放置图片
- 准备好你的高清壁纸图片（推荐使用 `.webp` 格式以获得更快的加载速度，也支持 `.jpg` 或 `.png`）。
- 将图片文件上传或移动到项目的 `/public/assets/` 目录下。例如，文件名存为 `my-wallpaper.webp`。
...
export const PRESET_WALLPAPERS = [
  // ... 保留现有的预设 ...
  {
    id: "my-custom-bg",       // 唯一 ID（不可重复）
    name: "我的独家壁纸",      // 在控制中心显示的名称
    url: "my-wallpaper.webp",  // 你刚刚放入 /public/assets/ 的文件名
    thumbnail: "my-wallpaper.webp", // 缩略图（通常与原图一致即可）
    isDark: true              // 如果壁纸色调较暗，请设为 true，系统会自动适配浅色文字
  }
];
```

### 3. 查看效果
- **本地开发**：如果你正在运行 `npm run dev`，主页会实时刷新。打开右下角的**控制中心 -> 壁纸图标**，你就能看到新添加的壁纸了。
- **已部署环境**：修改代码并推送（Git Push）到你的仓库，部署平台会自动重新编译并发布新版本。

> **💡 小贴士**：除了手动修改代码外，你也可以直接在主页的“控制中心 -> 壁纸设置 -> 自定义网址”中，直接输入网络图片的链接进行即时更换（此操作仅保存在您的本地浏览器中）。
